...
public class VideoInformation {
	final static String TAG = "VideoInformation";
...
//				Log.d(TAG,name+"="+value);
				...
				if( name.equals("video_id") ){
					...
					try{
						...
						MessageFormat mf = new MessageFormat("{0,number,integer}-{1,number,integer}-{2,number,integer}T{3,number,integer}:{4,number,integer}:{5,number,integer}");
						...
						mFirstRetrieve = ""+y +"/"+(mon<10?"0"+mon:mon)+"/"+(d<10?"0"+d:d);
						mFirstRetrieve +=" "+h+":"+(min<10?"0"+min:min)+":"+(s<10?"0"+s:s);
					}catch(Exception e){
					...
					try{
						MessageFormat mf = new MessageFormat("{0,number,integer}:{1,number,integer}");
						Object[] result = mf.parse(value);
					...
					if( attrs.getNamedItem("domain").getTextContent().equals("jp") ){
						int sz = node.getChildNodes().getLength();
						mTags = new String[sz];
						for(int i=0; i<sz; i++){
							mTags[i] = node.getChildNodes().item(i).getTextContent();
						}
					}
